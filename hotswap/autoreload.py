...
class ModuleReloader(Atom):
    ...
    def filename_and_mtime(self, module):
        ...
        if ext.lower() in self.source_exts:
...
class Autoreloader(Atom):
    ...
    #: Reloader
    _reloader = Instance(ModuleReloader)

    loaded_modules = Instance(set)

    def _default__reloader(self):
        return ModuleReloader(check_all=False)
